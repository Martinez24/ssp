<?php
    ...
    **/
    ...
    if($_POST['agrega_proyecto']){
        extract($_POST);
        $sql = "SELECT  m.modelo, m.nombre as maquina, c.nombre, c.estado as destino, pr.id_proyecto, pr.fecha_entrega, pr.fecha_creacion, pr.etiqueta as etiqueta from proyecto pr INNER JOIN cliente c on c.id_cliente = pr.id_cliente INNER JOIN maquina m on m.id_maquina = pr.id_maquina WHERE id_proyecto = $proyecto_id";
   ...
   if($_POST['agrega_tarea']){
        ...
        $gestion['motor'] = $motor;
        $gestion['no_ejes'] = $no_ejes;
        $gestion['descripcion_m'] = $descripcion_m;
    ...
    if(isset($_GET['guardar_tarea'])){
        extract($gestion=$tarea['gestiones']);  
         ...
         // Por cada departamento guardaremos un registro relacionado al proyecto
            $sql = "INSERT INTO tarea(estatus, id_departamento, id_personal, id_proyecto, fecha_inicio, fecha_entrega, observacion, reporte)
            VALUES(1,".$gestion['id_departamento'].",".$gestion['id_personal'].",".$gestion['id_proyecto'].", '".$gestion['fecha_inicio']."', '".$gestion['fecha_entrega']."', '".$gestion['observacion']."', '".$gestion['reporte']."')";
            $resultado = mysql_query($sql, $conexion); 
